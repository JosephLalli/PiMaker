...
def process_gene(read_cts, sample_consensus_seqs, muts_in_gene, synon_cts, SNP_freqs, genePiMathArray, sample_list,
    ...
    # sample_consensus_seqs = calc_consensus_seqs(read_cts, ref_array, muts_in_gene)
    # mut_codons = np.unravel_index(muts_in_gene, (len(gene_slice)//3,3))[0]
    # refSeqCodons = sample_consensus_seqs.reshape(len(sample_list), -1, 12)
    # var_codons = refSeqCodons[:,mut_codons,:]
    # synon_filters = generate_coding_filters(sample_consensus_seqs.reshape(len(sample_list), -1, 12)[:,mut_codons,:], *synon_cts, idx_of_var_sites_in_gene=muts_in_gene)
